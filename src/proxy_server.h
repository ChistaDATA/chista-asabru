...
int updateProxyConfig(CProxySocket *socket, RESOLVED_PROXY_CONFIG configValue) {
	TARGET_ENDPOINT_CONFIG targetEndpointConfig = {
		.name = configValue.name, .proxyPort = configValue.proxyPort, .services = configValue.services};
			...
			if (endpoint.endPointName == endpointServiceConfig.name) {
				if (endpointServiceConfig.operation == "add") {
					...
					auto end_it = std::remove_if(
						...
						[&endpointServiceConfig](const SERVICE &service) { return service.name == endpointServiceConfig.name; });
...
int startLibuvProxyServer(LibuvProxySocket *socket, PipelineFunction<LibuvProxySocket> pipelineFunction,
...
int startProxyServer(CProxySocket *socket, RESOLVED_PROXY_CONFIG configValue) {
	// Create proxies
	std::string proxyName = configValue.name;
...
int initProxyServers() {
	...
	for (const auto &value : configValues) {
		if (value.pipelineName == "ClickHouseLibuvPipeline") {
			int proxy = startLibuvProxyServer(new LibuvProxySocket(value.proxyPort),
											  pipelineFactory.GetPipeline<LibuvProxySocket>(value.pipelineName), value);
			if (proxy < 0)
				return proxy;
		} else {
			...
			int proxy = startProxyServer(configSingleton.proxySocketsMap[value.name], value);
			if (proxy < 0)
				return proxy;
		}
	}
